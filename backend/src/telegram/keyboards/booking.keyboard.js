...
export const BOOKING_CB = {
  ...
  DAY_SELECT: "crm_booking:day_select", // crm_booking:day_select:<YYYY-MM-DD>
...
export const bookingPeriodKeyboard = () => {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback("⬅️ Назад", "crm_back:booking_level")],
  ])
}

export const bookingNext7DaysKeyboard = ({ dates }) => {
  const row = (dates || []).slice(0, 7).map((d) =>
    Markup.button.callback(d.slice(8, 10), `${BOOKING_CB.DAY_SELECT}:${d}`)
  )

  return Markup.inlineKeyboard([
    row,
    [Markup.button.callback("⬅️ Назад", "crm_back:booking_period")],
  ])
}
...
export const bookingListKeyboard = ({ type, page, hasPrev, hasNext, items }) => {
  ...
  for (const b of items) {
    const title = type === "day"
      ? `🕒 ${b.time} • ${b.name}`
      : `📅 ${formatDate(b.date)} 🕒 ${b.time} • ${b.name}`
    rows.push([
  ...
  const nav = []
  if (hasPrev) nav.push(Markup.button.callback("⬅️ Пред.", `${BOOKING_CB.LIST}:${type}:${page - 1}`))
  if (hasNext) nav.push(Markup.button.callback("➡️ След.", `${BOOKING_CB.LIST}:${type}:${page + 1}`))
  if (nav.length) rows.push(nav)

  // one inline back per screen
  const backTarget =
    type === "day"
      ? "crm_back:booking_day_picker"
      : "crm_back:booking_period"
  rows.push([Markup.button.callback("⬅️ Назад", backTarget)])
...
export const bookingCardKeyboard = ({ bookingId, type, page }) => {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback("⬅️ Назад", "crm_back:booking_list")],
  ])
}
...
export const rescheduleDatesKeyboard = ({ dates }) => {
  ...
  for (let i = 0; i < dates.length; i += perRow) {
    ...
    rows.push(
      chunk.map((d) => Markup.button.callback(formatDate(d), `${BOOKING_CB.RESCHEDULE_DATE}:${d}`))
    )
  }

  rows.push([Markup.button.callback("⬅️ Назад", "crm_back:booking_card")])
...
export const rescheduleTimesKeyboard = ({ times }) => {
  ...
  rows.push([Markup.button.callback("⬅️ Назад", "crm_back:reschedule_dates")])
...
export const rescheduleConfirmKeyboard = () => {
  return Markup.inlineKeyboard([
    [
      ...
      Markup.button.callback("⬅️ Назад", "crm_back:reschedule_times"),
    ],
  ])
}
